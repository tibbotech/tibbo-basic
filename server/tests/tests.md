...
| `implicitconv` | Decimal string literals folded to `word` / `short` in `dim` and assignment (matches tmake `LOA8I` / `LOA16I` immediates; `dword` string init differs and is omitted) | `main.tbs`, `global.tbh`, `blank.tpr` |
...
| `syscalls` | `sys` getters/setters (`timercount`, `runmode`, `timercount32`, `onsystimerperiod`, `wdperiod`), `str`/`lstr`/`stri`/`lstri`, `hex`/`bin`/`lhex`/`lbin`, folded `chr`, nested `str(val(var))` (tmake uses `LEA` at str output temp + one `TEMP_STRING_SLOT_SIZE` for `val`'s byref + matching `LOA16I`), void `sys.halt` (no `sys.sleep` on this platform) | `main.tbs`, `global.tbh`, `blank.tpr` |
