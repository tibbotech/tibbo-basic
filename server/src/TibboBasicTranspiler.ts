...
export class TibboBasicTranspiler {
    ...
    variables: { [name: string]: TBVariable } = {};
    lineMappings: number[] = [];
...
class ParserListener extends TibboBasicParserListener {
    ...
    assignsByrefVariable(ctx: any, paramName: string, isAssignment = false) {
        if (!ctx.children) {
            return isAssignment;
        }
