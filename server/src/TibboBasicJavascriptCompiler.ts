...
export class TibboBasicJavascriptCompiler {
    ...
    lineMappings: number[] = [];
    constants: { [name: string]: TibboConstant } = {};
...
class ParserListener extends TibboBasicParserListener {
    ...
    compiler: TibboBasicJavascriptCompiler;
    ...
    constructor(compiler: TibboBasicJavascriptCompiler) {
        super();
            ...
            default:
                {
                    ...
                    if (variableType.match(exp) !== null) {
                        const matchResult = variableType.match(exp);
                        const varType = matchResult ? matchResult[1] : '';
    ...
    findVariable(name: string, ctx: any) {
    ...
    exitFunctionStmt(ctx: any) {
        this.compiler.addCode('}', ctx.stop.line);
        this.compiler.writeLine(ctx.stop.line);
        this.compiler.appendLine(`return ${this.currentFunction?.name};`, ctx.stop.line - 1);
        this.isDeclaration = false;
        this.currentFunction = undefined;
    }
    ...
    enterVariableListStmt(ctx: any) {
        ...
        for (let i = 0; i < ctx.children.length; i++) {
            ...
            if (item.ruleIndex == TibboBasicParser.RULE_arrayLiteral) {
                ...
                tmpCode = tmpCode.replace(/\}/gm, "");
                const parts: string[] = tmpCode.split(',');
    ...
    parseExpression(ctx: any, isAssignment = false) {
        let result = '';
        for (let i = 0; i < ctx.children.length; i++) {
            const item = ctx.children[i];
            if (item.symbol) {
    ...
    enterExitStmt(ctx: any) {
        switch (ctx.children[0].symbol.type) {
            ...
            case TibboBasicLexer.EXIT_FUNCTION:
                this.compiler.addCode(`return ${this.currentFunction?.name};`, ctx.start.line);
                this.compiler.writeLine(ctx.start.line);
                break;
        }
    }
    ...
    enterObjectDeclaration(ctx: any) {
    ...
    enterPropertyDefineStmt(ctx: any) {
    ...
    enterSyscallDeclarationInner(ctx: any) {
        this.isDeclaration = false;
        if (ctx.object != null) {
            const objectName = ctx.object.text;
    ...
    exitSyscallDeclarationInner(ctx: any) {
        if (this.currentFunction != undefined) {
