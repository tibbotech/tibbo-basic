...
connection.onInitialize(async (params: InitializeParams) => {
    ...
    if (workspaceRoot) {
        fs.readdirSync(workspaceRoot).forEach((file: any) => {
...
connection.onDocumentFormatting((formatParams: any) => {
            ...
            case '.':
                {
                    ...
                    if (token != undefined) {
                        ...
                        if (obj != undefined) {
                            obj.functions.forEach((func: any) => {
                                suggestions.push({
                                    label: func.name,
...
connection.onHover(({ textDocument, position }: any): Hover | undefined => {
            ...
            case TibboBasicParser.IDENTIFIER:
                ...
                if (result.value != '') {
                    ...
                    if (func.location != undefined) {
                        result.value = '```tibbo-basic\n';
                        result.value += `sub ${func.name}(${func.parameters.map((param: any) => {
                    ...
                    const syscall = projectParser.syscalls[text];
                    result.value = '```tibbo-basic\n';
                    result.value += `syscall ${syscall.name}(${syscall.parameters.map((param: any) => {
                        return `${param.byref ? 'byref' : ''} ${param.name} as ${param.dataType}`;
                    }).join(',')})`;
                    if (syscall.dataType != '') {
...
connection.onDeclaration(({ textDocument, position }: any): Declaration | undefined => {
    const document = documents.get(textDocument.uri);
    if (!document) {
        return;
    }
...
connection.onDefinition(({ textDocument, position }: any): Definition | undefined => {
    const document = documents.get(textDocument.uri);
    if (!document) {
        return;
    }
...
connection.onDocumentSymbol(({ textDocument }: any): DocumentSymbol[] | undefined => {
    const document = documents.get(textDocument.uri);
    if (!document) {
        return;
    }
...
function copyProperties() {
    ...
    let obj: any = platformProjectParser.tokens;
    for (const key in obj) {
